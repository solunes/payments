...
class PaypalPaymentController extends Controller {
    ...
    public function getMakeAllPayments($customer_id, $custom_app_key = NULL) {
        ...
        if($customer){
          ...
          $paypal_transaction = \Paypal::generatePaymentTransaction($customer_id, $payment_ids, $total_amount);
          $final_fields = \Paypal::generateTransactionArray($customer, $payment, $paypal_transaction, $custom_app_key);
          $api_url = \Paypal::generateTransactionQuery($paypal_transaction, $final_fields);
          if($api_url){
    ...
    public function getMakeSinglePayment($customer_id, $payment_id, $custom_app_key = NULL) {
        ...
        if($customer&&$payment){
          $paypal_transaction = \Paypal::generatePaymentTransaction($customer_id, [$payment_id], $payment['amount']);
          $final_fields = \Paypal::generateTransactionArray($customer, $payment, $paypal_transaction, $custom_app_key);
          $api_url = \Paypal::generateTransactionQuery($paypal_transaction, $final_fields);
          if($api_url){
            return redirect($api_url);
